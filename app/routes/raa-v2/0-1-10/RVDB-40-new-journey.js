...
function removeFromList(array,value){
  ...
  router.post('/' + base_url + '*/training-level*', function (req, res) {
    res.redirect(301, '/' + base_url + req.params[0]+"/training-confirm")
  })
  router.post('/' + base_url + '*/training-confirm', function (req, res) {
    if(req.session.data.user=="employer"){
  ...
  router.post('/' + base_url + '*/closing', function (req, res) {
    // if(req.body.disability_confident_signed=='yes'){
    //
    // }
    if(req.session.data.edit == "yes"){
      ...
      res.redirect(301, '/'   + base_url + req.params[0]+"/preview-start?edit=no")
    }
  })
  ...
  router.post('/' + base_url + '*/application-process', function (req, res) {
    if(req.session.data.edit == "yes"){
      ...
      res.redirect(301, '/'   + base_url + req.params[0]+"/closing?edit=no")
    }
