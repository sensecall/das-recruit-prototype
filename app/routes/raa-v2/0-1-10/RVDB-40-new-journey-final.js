module.exports = function (router) {
  ...
  var version = '0-1-9'
  var base_url = 'raa-v2/' + version + '/RVDB-40-new-journey-final';
...
function getTrainingList($query,$data){
  ...
  router.post('/' + base_url + '*/wages', function (req, res) {
    var minWage = Math.round((req.session.data.weeklyHours * 52)*req.session.data.nationalMinWage)
    if(req.session.data.WageType == "nationalMinWage"){
      ...
      req.session.data.yearlyWage = "£"+req.body.FixedWageYearlyAmount
      console.log(req.body.FixedWageYearlyAmount)
      console.log(minWage)
      if(parseInt(req.body.FixedWageYearlyAmount) < minWage ){
        res.redirect(301, '/' + base_url + req.params[0]+"/wages?error=wages");
