...
function getTrainingList($query,$data){
  ...
  router.get('/' + base_url + '*/training', function (req, res) {
    req.session.data.title = req.session.data.title || "Nurse Associate Apprentice (West Norfolk)"
    var query = (req.query.query) ? req.query.query.replace('Apprenticeship','') : req.session.data.title.replace('Apprenticeship','')
    var query2 = query.replace('Apprenticeship','').replace('apprenticeship','').replace('apprentice','').replace('Apprentice','')
    console.log(query2)
    var options = {
        shouldSort: true,
        threshold: 0.5,
