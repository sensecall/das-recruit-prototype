function checkboxEditing(totalCheckboxesCount,clearLocalStorage,itemsPerPage,itemTypeLabel,itemTypeLabelPlural){
    ...
    var testCount = 0
    // TODO possible bug where ticked boxes not triggering saving of local storage sometimes
    ...
    var _checkboxes = _checkboxesContainer.find("input[type='checkbox']")
    ...
    jQuery.each(tickedCheckboxes, function(key, value) {
        ...
        jQuery("#" + key).closest("label").addClass("selected")
    });
    ...
    _selectAllLink.on(
        ...
        function(event){
            ...
            if (jQuery("[data-checkboxes]").find("input[type='checkbox']:checked").length == _checkboxes.length) {
                _checkboxes.prop('checked', false).closest("label").removeClass("selected");
            } else {
                _checkboxes.prop('checked', true).closest("label").addClass("selected");
            }
    ...
    _search.on(
        ...
        function(event){
            ...
            // _checkboxes.closest("label").show()
            // _checkboxes.data("matches-search") = true
            _checkboxes.data("matches-search", true)
            showOrHideElement(_checkboxes.closest("label"), "show", true)
            // _checkboxes.closest("label").show()
    ...
    function doPaging(reset){
        ...
        _checkboxes.each(function(index) {
            ...
            if((_this.data("matches-search") == true) && ((_displayedCount > _pagingCalc1) && (_displayedCount <= _pagingCalc2))){
                showOrHideElement(_this.closest("label"), "show", true)
                ...
                showOrHideElement(_this.closest("label"), "hide", true)
            }
